        ...
        for i in range(0,n):
            for j in range(0,n):
                if state[i,j]>1:# is empty
                    ...
                    nextstate[i,j]=1
                    Eval,Nstate=minimax(nextstate, not isMaxmazing)
...
def main():
    ...
    while True:
        ...
        hvalue, state= minimax(board,False)
        ...
        fileo=open(filename_o,'a')
        fileo.write('o'+str(row+1)+colname+'\n')
        fileo.close()
        print("o plays:",'o'+str(row+1)+colname+'\n')
